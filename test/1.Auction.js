  ...
  describe("Real Estate Auction", () => {
    ...
    it("Only owner can start Auction", async () => {
      await expect(
        auction
          .connect(bidder1)
          .startAuction(ethers.utils.parseEther("1"), 3600 * 24 * 7)
      ...
      await expect(
        auction.startAuction(ethers.utils.parseEther("1"), 3600 * 24 * 7)
    ...
    describe("Start Auction", () => {
      ...
      it("Owner cannot participate in bidding", async () => {
        await expect(
          auction.bid({ value: ethers.utils.parseEther("2") })
        ).to.be.revertedWith("Permission Denied");
      });
      ...
      it("Current bid should be higher than the last bid", async () => {
        ...
        ).to.be.revertedWith("Bid more than the latest bid");
      });

      it("Get latest bid function works", async () => {
        expect(await auction.highestBid()).to.be.equal(
      ...
      describe("End Auction", () => {
        ...
        it("The owner gets the highest bid", async () => {
          ...
          expect(
            await auction
              .connect(bidder1)
              .bid({ value: ethers.utils.parseEther("3") })
          ).to.be.ok;

          expect(await ethers.provider.getBalance(auction.address)).to.be.equal(
          ...
          let expectedBalance = sellerPreviousBalance
            .add(ethers.utils.parseEther("3"))
            .toString()
            .slice(0, 5);

          expect(sellerCurrentBalance.toString().slice(0, 5)).to.be.equal(
            expectedBalance
          );
        });

        it("The highest bidder gets the nft transferred", async () => {
          expect(await excelApparment.ownerOf(nftId)).to.be.equal(
            auction.address
          );
          expect(
            await auction
      ...
      describe("Withdraw funds", () => {
        it("Bidders can withdraw their amount", async () => {
          ...
          expect(
            await auction
              .connect(bidder2)
              .bid({ value: ethers.utils.parseEther("4") })
          ).to.be.ok;
          ...
          let bidder1ExpectedBalance = bidder1PreviousBalance
            .add(ethers.utils.parseEther("2"))
            .toString()
            .slice(0, 5);
          expect(bidder1CurrentBalance.toString().slice(0, 5)).to.be.equal(
            bidder1ExpectedBalance
          );
          ...
          let bidder2ExpectedBalance = bidder2PreviousBalance
            .add(ethers.utils.parseEther("3"))
            .toString()
            .slice(0, 5);
          expect(bidder2CurrentBalance.toString().slice(0, 5)).to.be.equal(
            bidder2ExpectedBalance
          );
