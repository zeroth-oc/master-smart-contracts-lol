...
describe("Staking Rewards", () => {
    ...
    it("Update Duration and Update reward works", async () => {
        expect(await rewardToken.balanceOf(staking.address)).to.be.equal(ethers.utils.parseEther("10000"));

        await expect(staking.updateRewardAmount(ethers.utils.parseEther("12"))).to.be.rejectedWith("Duration is not set");
        ...
        await expect(staking.setDuration(60 * 60)).to.be.rejectedWith("Reward not finished yet");
        await expect(staking.updateRewardAmount(ethers.utils.parseEther("0"))).to.be.rejectedWith("Reward amount should be greater than zero");
        expect(await staking.rewardRate()).to.be.equal(ethers.utils.parseEther("2"));
    ...
    it("Staking works", async () => {
        ...
        await stakingToken.connect(account1).approve(staking.address, ethers.utils.parseEther("1000"));
        ...
        expect(await staking.balanceOf(account1.address)).to.be.equal(ethers.utils.parseEther("1000"));
        expect(await staking.userRewardPerTokenPaid(account1.address)).to.be.equal(0);
        
        await stakingToken.mint(account2.address, ethers.utils.parseEther("2000"));
        await stakingToken.connect(account2).approve(staking.address, ethers.utils.parseEther("2000"));
        ...
        expect(await staking.balanceOf(account2.address)).to.be.equal(ethers.utils.parseEther("2000"));
        expect(await staking.totalSupply()).to.be.equal(ethers.utils.parseEther("3000"));
    });
    ...
    it("Withdraw works for single staker", async () => {
        ...
        expect(await stakingToken.balanceOf(account1.address)).to.be.equal(ethers.utils.parseEther("1000"));
        let expectedReward = (finishedTimestamp-stakeTimestamp)*2;
        expect(await staking.earned(account1.address)).to.be.equal(ethers.utils.parseEther(expectedReward.toString()));
        await staking.getEarned();
        expect(await rewardToken.balanceOf(account1.address)).to.be.equal(ethers.utils.parseEther(expectedReward.toString()));
    });
    ...
    it("Withdraw works for multiple stakers", async () => {
        ...
        expect(await stakingToken.balanceOf(account2.address)).to.be.equal(ethers.utils.parseEther("2000"));
