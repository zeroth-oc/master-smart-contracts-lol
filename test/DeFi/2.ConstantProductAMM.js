...
describe("Vault", () => {
  ...
  it("Initial Add liquidity works", async () => {
    ...
    await expect(
      marketMaker.connect(account1).depositLiquidity(2000, 1000)
    ).to.be.revertedWith("x/y != dy/dx");
  });
  ...
  it("Add liquidity works after many swaps", async () => {
    ...
    await marketMaker
      .connect(account1)
      .depositLiquidity(reserve1 * 0.5, reserve2 * 0.5);
  ...
  it("withdraw works", async () => {
    ...
    expect(await token1.balanceOf(account1.address)).to.be.equal(
      reserve1 * 0.5
    );
    expect(await token2.balanceOf(account1.address)).to.be.equal(
      reserve2 * 0.5
    );
