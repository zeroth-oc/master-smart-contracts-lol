...
describe("Vault", () => {
    ...
    it("Deposit works", async () => {
        ...
        expect(await vault.connect(account1).balanceOf(account1.address)).to.be.equal(10000);
        ...
        expect(await vault.connect(account2).balanceOf(account2.address)).to.be.equal(1000);
    })
    ...
    it("Withdraw issue", async () => {
        ...
        await token.mint(account1.address, 10000);
        await expect(vault.connect(account2).withdraw(1000)).to.be.revertedWith("Not enough shares");
        await token.mint(account2.address, 1000);
        ...
        await expect(vault.connect(account2).withdraw(1100)).to.be.revertedWith("Not enough shares");
        expect(await vault.balanceOf(account2.address)).to.be.equal(1000);
        expect(await token.balanceOf(account2.address)).to.be.equal(0);
    })
