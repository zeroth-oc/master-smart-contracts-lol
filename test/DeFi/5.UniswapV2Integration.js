...
describe("Uniswap V2", () => {
  ...
  it("uniswap single swap with exact no of tokens in works", async () => {
    ...
    let txn = await uniswap.singleSwapExactAmountIn(
  ...
  it("uniswap muliple swap with exact no of tokens in works", async () => {
    ...
    console.log(
      "LINK - ",
      ethers.utils.formatEther(await link.balanceOf(account.address))
    );
  });
  ...
  it("uniswap single swap with exact no of tokens out works", async () => {
    ...
    await deposit.wait();
    await weth.approve(uniswap.address, ethers.utils.parseEther("10"));
    let txn = await uniswap.singleSwapExactAmountOut(
      1500 * 10 ** 6,
      ethers.utils.parseEther("10"),
      {
        gasLimit: 1000000,
  ...
  it("uniswap add and remove liqidity works", async () => {
    ...
    console.log(
      "WETH - ",
      ethers.utils.formatEther(await weth.balanceOf(uniswap.address))
    );
    ...
    expect(
      (await liquidityToken.balanceOf(uniswap.address)) / 10 ** 18
    ).to.be.greaterThan(0);
  ...
  it("Flash swap works", async () => {
    ...
    console.log(
      "Fee Approval - ",
      await weth.allowance(account.address, uniswap.address)
    );
    await uniswap.flashSwap(ethers.utils.parseEther("10000"), {
      gasLimit: 1000000,
    });
  });
});
