...
describe("Self Desctruct Attack", () => {
  ...
  describe("Ticket Booking", () => {
    it("Can buy tickets with 1 ether", async () => {
      ...
      expect(
        await ticketBooking.buyTicket({ value: ethers.utils.parseEther("1") })
      ).to.be.ok;
      expect(await ticketBooking.getBalance()).to.be.equal(
        ethers.utils.parseEther("1")
      );
      expect(await ticketBooking.havePurchased(account.address)).to.be.equal(
        true
      );
    });
    ...
    it("Reverts when amount sent is more or less than 1 ether", async () => {
      await expect(
        ticketBooking.buyTicket({ value: ethers.utils.parseEther("0.5") })
      ).to.be.revertedWith("Need to pay 1 ether");
      await expect(
        ticketBooking.buyTicket({ value: ethers.utils.parseEther("1.5") })
      ).to.be.revertedWith("Need to pay 1 ether");
    });
  });
  ...
  describe("Attack", () => {
    it("Forcefully send ether with selfdesctruct", async () => {
      ...
      expect(
        await attacker.attack(ticketBooking.address, {
      ...
      await expect(
        ticketBooking.buyTicket({ value: ethers.utils.parseEther("1") })
      ).to.be.revertedWith("Sorry. All tickets sold.");
