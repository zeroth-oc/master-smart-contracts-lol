...
describe("Front Running - Vulnerable", () => {
  ...
  describe("Solve hash works properly", () => {
    it("Guess Hash", async () => {
      await expect(solveHash.solve("test")).to.be.rejectedWith(
        "Incorrect guess"
      );

      let previousBalance = await account1.getBalance();
      expect(await solveHash.solve("Gowtham is a smart contract master")).to.be
        .ok;
...
describe("Front Running - Using Commit Reveal as prevention", () => {
  ...
  beforeEach(async () => {
    ...
    solveHash = await SolveHash.deploy({
  ...
  describe("Solve hash with commit and reveal", () => {
    it("Commit and get guess works", async () => {
      let guesshash = ethers.utils.keccak256(
        ethers.utils.toUtf8Bytes(account1.address.toLowerCase() + word + secret)
      );
      console.log("hash", guesshash);
      expect(await solveHash.commitSolution(guesshash)).to.be.ok;

      await expect(solveHash.commitSolution(guesshash)).to.be.revertedWith(
        "Already committed"
      );
    ...
    it("Reveal guess works", async () => {
      ...
      await expect(solveHash.revealSolution(word, secret)).to.be.revertedWith(
        "Not committed yet"
      );
