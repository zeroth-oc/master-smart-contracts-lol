...
describe("Private Variables", () => {
    ...
    describe("Attack", () => {
        it("Read private and public data in the contract", async () => {
           ...
           let data = await ethers.provider.getStorageAt(secretData.address, paddedSlot);
           ...
           expect(await getShortStr(2, secretData.address)).to.be.equal("Secret data");
           expect(await getShortStr(3, secretData.address)).to.be.equal("Secret data1");

           paddedSlot = ethers.utils.hexZeroPad(4, 32);
           data = await ethers.provider.getStorageAt(secretData.address, paddedSlot);
           expect(parseInt(ethers.BigNumber.from(data.slice(60, 64)), 16)).to.be.equal(100);

           paddedSlot = ethers.utils.hexZeroPad(4, 32);
           data = await ethers.provider.getStorageAt(secretData.address, paddedSlot);
           expect(parseInt(ethers.BigNumber.from(data.slice(64, 68)), 16)).to.be.equal(1);
        });
    })
