...
describe("Arithmetic Overflow Underflow Attack", () => {
  ...
  describe("Timelock", () => {
    it("Can deposit amount", async () => {
      ...
      expect(await timelock.deposit({ value: ethers.utils.parseEther("10") }))
        .to.be.ok;
      expect(await timelock.getBalance()).to.be.equal(
        ethers.utils.parseEther("10")
      );
      expect(await timelock.balances(account.address)).to.be.equal(
        ethers.utils.parseEther("10")
      );
    });
    ...
    it("Can withdraw amount", async () => {
      ...
      await expect(timelock.withdraw()).to.be.revertedWith(
        "Lock time not completed"
      );
  ...
  describe("Attack", () => {
    it("Overflow error", async () => {
      await expect(attacker.attack({ value: ethers.utils.parseEther("10") })).to
        .be.reverted;
    });
  });
});
